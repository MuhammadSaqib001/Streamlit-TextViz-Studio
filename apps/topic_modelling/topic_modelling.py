...
st.set_page_config(page_title='Text2Topics', page_icon='🗂️', layout="wide", initial_sidebar_state="expanded")

# Sidebar: Title and description (sticky)
#st.sidebar.image("https://your-image-url-here.com", width=150)  # Optional logo
st.sidebar.title("Text2Topic Analysis")
st.sidebar.markdown("""
**Text2Topic Analysis** is an interactive tool for extracting and visualizing topics from text data.

- **Upload a CSV file** containing your text data for analysis.
- **Select topic generation preferences**: define a specific number of topics or let the model determine the optimal number.
- **Choose advanced options** like outlier reduction and the use of OpenAI's GPT-4o for improved topic labels.
- **Visualize the results** through a topic summary, intertopic distance map, and document-topic probabilities. Results can be downloade for further analyses of topics.

Configure the parameters to customize your topic modeling experience.
""")

st.sidebar.markdown("[Documentation](https://your-docs-url.com)")
...
umap_random_state = st.number_input("Enter a seed number for pseudorandomization (optional)", min_value=0, value=0, step=1)
